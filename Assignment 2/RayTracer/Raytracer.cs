...
namespace RayTracer
{
    public class Raytracer
    {
        ...
        public void Render(Surface screen)
        {
            ...
            //Cast rays 
            for (int x = 0; x < Camera.resolution; x++)
            {
                for (int y = 0; y < Camera.resolution; y++)
                {
        ...
        private Vector3 CalculateColor(Ray ray, int recursionDepth = 0)
        {
            ...
            if (intersection != null)
            {
                ...
                if (1 - diffuse > EPS)
                {
                    ...
                    Ray secondaryRay = new Ray(intersectionpoint, ray.direction - 2 * Vector3.Dot(ray.direction, normal) * normal);
                    specularpart = color * CalculateColor(secondaryRay, recursionDepth + 1);
                }
        ...
        private void DrawInitialDebug(Surface screen)
        {
            ...
            //Screenplane
            x = TXDebug(camera.screencorner_topleft.X);
            y = TYDebug(camera.screencorner_topleft.Z);
            int x2 = TXDebug(camera.screencorner_topright.X);
            int y2 = TYDebug(camera.screencorner_topright.Z);
